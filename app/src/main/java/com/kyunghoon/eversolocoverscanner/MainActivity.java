...
public class MainActivity extends AppCompatActivity {
    ...
    final Set<String> audioExt = new HashSet<>(Arrays.asList(
        "flac","dsf","dff","wav","aiff","aif","mp3","m4a","aac","ogg","ape","wv"));
    final Set<String> coverNames = new HashSet<>(Arrays.asList(
        "cover.jpg","cover.jpeg","cover.png","folder.jpg","folder.jpeg","folder.png",
        "front.jpg","front.jpeg","front.png"));

    @Override public void onCreate(Bundle b) {
        super.onCreate(b); setContentView(R.layout.activity_main);
        status=findViewById(R.id.status); result=findViewById(R.id.result);
    ...
    @Override protected void onActivityResult(int req,int res,Intent data) {
        ...
        if(req!=PICK_TREE || res!=RESULT_OK || data==null) return;
    ...
    private void scan(Uri treeUri) {
        ...
        runOnUiThread(() -> {
            ...
            result.setText("Audio tracks: "+s.audio+"\n"+
    ...
    private boolean walk(DocumentFile dir, Stats s) {
        DocumentFile[] kids;
        try { kids=dir.listFiles(); } catch(Exception e){ return false; }
        boolean folderHasAudio=false, external=false;
        ...
        for(DocumentFile f:kids) {
            ...
            if(audioExt.contains(ext)) { audios.add(f); folderHasAudio=true; }
        }
        if(folderHasAudio) {
            s.audioFolders++;
            s.audio += audios.size();
            if(external) {
                ...
                boolean anyEmbedded=false, anyUnknown=false;
                for(DocumentFile a:audios) {
                    int r=hasEmbedded(a.getUri());
                    if(r==1){ anyEmbedded=true; break; }
        ...
        return folderHasAudio;
    }

    // 1=embedded picture found, 0=none, -1=extractor could not inspect.
    ...
    private String extension(String n) {
        int p=n.lastIndexOf('.'); return p<0?"":n.substring(p+1);
    }
    static class Stats {
        long audio=0,audioFolders=0,externalCoverFolders=0,embedded=0,missing=0,unknown=0;
    }
}
